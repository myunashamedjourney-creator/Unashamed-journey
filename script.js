...
// Remember which clean social link brought the visitor in.
(function rememberAttribution(){
  const path = (window.location.pathname || '/').toLowerCase();
  const sources = ['instagram', 'tiktok', 'youtube'];
  const matched = sources.find(source =>
    path === `/${source}` || path === `/${source}-reset`
  );

  if (matched) {
    try {
      sessionStorage.setItem('uj_signup_source', matched);
      sessionStorage.setItem(
        'uj_signup_campaign',
        path.endsWith('-reset') ? '7day_reset' : 'profile'
      );
    } catch (_) {}
  }
})();
...
document.querySelectorAll('[data-waitlist]').forEach(form=>form.addEventListener('submit',async e=>{
  ...
  try {
    source=sessionStorage.getItem('uj_signup_source') || 'direct';
    campaign=sessionStorage.getItem('uj_signup_campaign') || 'website';
  } catch (_) {}
